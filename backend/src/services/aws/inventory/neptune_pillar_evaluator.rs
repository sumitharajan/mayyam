...
// Deterministic Neptune cluster inventory evaluators for the cost, resilience,
// and security pillars (roadmap rows 01-AWS-CLOUD-01450/01459/01486).
//
// Evaluates fields persisted by neptune_control_plane: engine_version, status,
// deletion_protection, multi_az, backup_retention_period, storage_encrypted,
// kms_key_id, member_count, iam_database_authentication_enabled,
// audit_logs_enabled, plus tags.
...
pub const RESOURCE_TYPE: &str = "NeptuneCluster";

pub const REASON_COST_NO_TAGS: &str = "NEPTUNE_COST_NO_TAGS";
pub const REASON_RES_NO_MULTI_AZ: &str = "NEPTUNE_RES_NO_MULTI_AZ";
pub const REASON_RES_LOW_BACKUP_RETENTION: &str = "NEPTUNE_RES_LOW_BACKUP_RETENTION";
pub const REASON_RES_SINGLE_MEMBER: &str = "NEPTUNE_RES_SINGLE_MEMBER";
pub const REASON_RES_NO_DELETION_PROTECTION: &str = "NEPTUNE_RES_NO_DELETION_PROTECTION";
pub const REASON_SEC_NOT_ENCRYPTED: &str = "NEPTUNE_SEC_NOT_ENCRYPTED";
pub const REASON_SEC_NO_IAM_AUTH: &str = "NEPTUNE_SEC_NO_IAM_AUTH";
pub const REASON_SEC_NO_AUDIT_LOGS: &str = "NEPTUNE_SEC_NO_AUDIT_LOGS";
pub const REASON_INV_STALE_DATA: &str = "NEPTUNE_INV_STALE_DATA";

const MIN_BACKUP_RETENTION_DAYS: i64 = 7;

pub fn evaluate_neptune_fleet(
...
fn evaluate_cost(resource: &AwsResourceModel, findings: &mut Vec<InventoryFinding>) {
    ...
    if tags_empty {
        findings.push(InventoryFinding {
            ...
            message: format!(
                "Neptune cluster {} has no tags; cost allocation cannot be assessed",
...
fn evaluate_resilience(resource: &AwsResourceModel, findings: &mut Vec<InventoryFinding>) {
    if !data_bool(&resource.resource_data, "multi_az").unwrap_or(true) {
        findings.push(InventoryFinding {
            ...
            message: format!(
                "Neptune cluster {} is not Multi-AZ; a single-AZ cluster cannot fail over automatically during a zone outage",
                resource.resource_id
            ),
    ...
    if let Some(retention) = data_i64(&resource.resource_data, "backup_retention_period") {
        if retention < MIN_BACKUP_RETENTION_DAYS {
            findings.push(InventoryFinding {
                ...
                severity: Severity::Medium,
                message: format!(
                    "Neptune cluster {} backup retention is {} day(s); increase to at least {} days",
    ...
    if let Some(count) = data_i64(&resource.resource_data, "member_count") {
        if count < 2 {
            findings.push(InventoryFinding {
                ...
                severity: Severity::Medium,
                message: format!(
                    "Neptune cluster {} has only {} member instance(s); add a reader for faster failover",
    ...
    if !data_bool(&resource.resource_data, "deletion_protection").unwrap_or(false) {
        findings.push(InventoryFinding {
            ...
            severity: Severity::Medium,
            message: format!(
                "Neptune cluster {} does not have deletion protection enabled",
                resource.resource_id
            ),
...
fn evaluate_security(resource: &AwsResourceModel, findings: &mut Vec<InventoryFinding>) {
    if !data_bool(&resource.resource_data, "storage_encrypted").unwrap_or(false) {
        findings.push(InventoryFinding {
            ...
            severity: Severity::High,
            message: format!(
                "Neptune cluster {} storage is not encrypted at rest",
                resource.resource_id
            ),
    ...
    .unwrap_or(false)
    {
        findings.push(InventoryFinding {
            ...
            severity: Severity::Medium,
            message: format!(
                "Neptune cluster {} does not have IAM database authentication enabled; IAM auth removes long-lived password credentials",
                resource.resource_id
            ),
    ...
    if !data_bool(&resource.resource_data, "audit_logs_enabled").unwrap_or(false) {
        findings.push(InventoryFinding {
            ...
            message: format!(
                "Neptune cluster {} does not have audit logging enabled; enable audit logs to CloudWatch Logs for access tracking",
                resource.resource_id
            ),
...
mod tests {
    ...
    fn cost_flags_untagged_cluster() {
        ...
        let report = evaluate_neptune_fleet(&[r], Pillar::Cost, now());
    ...
    fn resilience_flags_no_multi_az() {
        ...
        let r = fixture("single-az", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Resilience, now());
    ...
    fn resilience_flags_low_backup_retention() {
        let mut data = healthy_data();
        data["backup_retention_period"] = json!(1);
        let r = fixture("low-backup", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Resilience, now());
    ...
    fn resilience_flags_single_member() {
        ...
        let r = fixture("single-member", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Resilience, now());
    ...
    fn resilience_flags_no_deletion_protection() {
        ...
        let r = fixture("no-del-prot", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Resilience, now());
    ...
    fn security_flags_unencrypted_storage() {
        ...
        let r = fixture("unencrypted", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Security, now());
    ...
    fn security_flags_no_iam_auth() {
        ...
        let r = fixture("no-iam", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Security, now());
        assert!(codes(&report).contains(&REASON_SEC_NO_IAM_AUTH));
    }
    ...
    fn security_flags_no_audit_logs() {
        ...
        let r = fixture("no-audit", json!({"team": "graph"}), data, now());
        let report = evaluate_neptune_fleet(&[r], Pillar::Security, now());
        assert!(codes(&report).contains(&REASON_SEC_NO_AUDIT_LOGS));
    }

    #[test]
    fn stale_resource_is_flagged() {
        let mut r = fixture("stale", json!({"team": "graph"}), healthy_data(), now());
    ...
    #[test]
    fn non_neptune_resources_are_skipped() {
