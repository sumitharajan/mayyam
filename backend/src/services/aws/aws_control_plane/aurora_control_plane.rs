...
pub struct AuroraControlPlane {
    aws_service: Arc<AwsService>,
}

impl AuroraControlPlane {
    ...
    pub async fn sync_clusters(
        ...
        debug!(
            "Syncing Aurora clusters for account: {} with sync_id: {}",
        ...
        loop {
            // Filter to Aurora engines only; build() is infallible in this SDK version.
            ...
            let response = request.send().await.map_err(|e| {
                error!("Failed to describe Aurora clusters: {}", e);
                AppError::ExternalService(format!("Failed to describe Aurora clusters: {}", e))
            })?;
            ...
            for cluster in response.db_clusters() {
                ...
                if !azs.is_empty() {
                    resource_data.insert("availability_zones".to_string(), json!(azs));
                }
                // engine_mode() returns Option<&str> directly
                if let Some(mode) = cluster.engine_mode() {
                    resource_data.insert("engine_mode".to_string(), json!(mode));
                }
                ...
                let dto = AwsResourceDto {
                    ...
                    resource_type: AwsResourceType::AuroraCluster.to_string(),
        ...
        debug!(
            "Successfully synced {} Aurora clusters for account: {} with sync_id: {}",
            resources.len(),
